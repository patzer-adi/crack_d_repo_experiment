...
This is a multi-machine project (synced via git). When making changes that future-you should remember on either machine, write them into the project files (committed to git), not into local Codex memory.

## When working on lessons

- **Batch generation (1–5 problems):** the user invokes `/batch-lesson <slug1> <slug2> ...`. The full workflow lives in [`.Codex/commands/batch-lesson.md`](.Codex/commands/batch-lesson.md) — read that when the command fires.
